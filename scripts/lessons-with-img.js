document.addEventListener("DOMContentLoaded", () => {
  ...
  for (let lesson of dataLessons) {
    ...
    newLesson.innerHTML = `
        ...
        <span class="new-lesson__img"></span>
        ...
        <span class="new-lesson__button">${
          lesson.state === "user-state-not_reached"
            ? "Недоступен"
            : "Смотреть урок"
        }</span>
      `;
    newLesson.querySelector(".new-lesson__img").style.cssText = `
      background: ${lesson.itemImg} center center / cover no-repeat;
      width: 100%;
      height: 94px;
      border-radius: 8px;
    `;
